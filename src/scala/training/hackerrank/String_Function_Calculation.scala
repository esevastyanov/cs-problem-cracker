...
/**
  ...
  */
object String_Function_Calculation extends App
{
  Solution.main(args)
  ...
  object Solution
  {
    ...
    def maxValue(s: String): Int = {
      SuffixArray.buildSuffixTree(s)
      SuffixArray.SuffixTreeNode.maxFunc
    }

    def main(args: Array[String]): Unit = {
      val sc = new java.util.Scanner(System.in)
      val string = sc.next()
      println(maxValue(string))
    }
    ...
    object SuffixArray
    {
      ...
      object SuffixTreeNode
      {
        ...
        def createNewLeaf(node: SuffixTreeNode, s: String, suffix: Int): SuffixTreeNode = {
          ...
          maxFunc = math.max(leaf.stringDepth - 1, maxFunc)
          if (node.children.size > 1) {
            var t = node
            while (t != null) {
              t.leafsNumber += 1
              maxFunc = math.max(t.leafsNumber * t.stringDepth, maxFunc)
              t = t.parent
            }
          }
          leaf
        ...
        def breakEdge(node: SuffixTreeNode, s: String, start: Int, offset: Int): SuffixTreeNode = {
          ...
          midNode.leafsNumber = node.children(startChar).leafsNumber
